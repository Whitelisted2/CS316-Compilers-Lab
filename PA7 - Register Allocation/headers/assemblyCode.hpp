	...
	void generateCode(CodeObject *code, std::vector<SymbolTable *> STvector)
	{
		...
		for (CodeLine *line : code->TAC)
		{
			...
			if (com == "STOREI" || com == "STOREF")
			{
				...
				assembly.push_back(CodeLine(line->scope, "move", getWhatever(farg1), tempReg));
				assembly.push_back(CodeLine(line->scope, "move", tempReg, getWhatever(farg2)));
			}
			...
			else
			{
				...
				for (std::string i : Ids)
				{
					if (i == com.substr(0, 3))
					{
						std::string nreg = getNewReg();
						assembly.push_back(CodeLine(line->scope, "move", getWhatever(farg1), nreg));
						assembly.push_back(CodeLine(line->scope, getLower(com), getWhatever(farg2), nreg));
						assembly.push_back(CodeLine(line->scope, "move", nreg, farg3));
						presentinId = true;
						break;
					}
				}
					...
					if (i == com)
					{
						...
						for (SymbolTable *table : STvector)
						{
							...
							if(table-> scope == "GLOBAL"){
								// std::cout<<"in global and need "<<farg1<<std::endl;
								// if(farg1 == "diff"){
								// 	// std::cout<<"confirmed"<<std::endl;
								// }
								Entry *entry1 = table->findEntry(farg1);
								// std::cout<<"need to find "<<farg1<<" and "<<farg2<<std::endl;
								Entry *entry2 = table->findEntry(farg2);
								// if(entry1){
								// 	// std::cout<<"found and type is "<<entry1->type<<std::endl;
								// }
								if ((entry1 && entry1->type == "FLOAT") || (entry2 && entry2->type == "FLOAT"))
								{
									globflag = 1;
